  ...
  <transition name="panel">
    <div ref="minSec" class="choose-panel-sec" v-if="showPanel">
      <div class="search-box">
        <input type="text" class="search-input" v-model="keyword" @keyup="search" placeholder="中文/拼音/首字母">
      </div>
      ...
      <section class="city-sec" v-if="!searchList.length">
        <div v-for="(item, index) in finCityList" :key="`cityList${index}`">
          ...
          <span @click="chooseCity(cityName.city)"
      ...
      <section class="city-sec" v-else>
        <div v-for="(item, index) in searchList" :key="`search${index}`">
          ...
          <span @click="chooseCity(cityName.city)" class="city-item" 
...
export default {
  ...
  setup (props, ctx) {
    const state = reactive({
      ...
      search: () => {
        if (state.keyword) {
          state.searchList = state.getFinCityList(state.fuzzySearch(state.keyword, props.cityList))
        } else {
          state.searchList = []
        }
      },
    ...
    watch(() => props.cityList,
      cityList => {
        state.finCityList = state.getFinCityList(cityList)
      }
    )
